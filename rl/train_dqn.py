...
import torch
import logging
from rl.clue_env import ClueEnv
from rl.dqn_agent import DQNAgent
from rl.reward_schedules import info_gain_reward_shaping

# ------------------------------------------------------------------
# Debug mode
VERBOSE = False  # Set True to print full game output for single-episode debugging
...
agent = DQNAgent(
    ...
    epsilon_end=0.05,
    epsilon_decay=0.997,
    accusation_epsilon_start=0.95,   # mostly wait during early training
...
# Wire the agent into the env so the reveal and accusation heads can be called
# from inside opponent turns and from the accusation decision point.
env.set_agent(agent)
...
if os.path.exists(latest_ckpt):
    ...
    print(f"Resumed from checkpoint at episode {start_episode}, "
          f"ε={agent.epsilon:.3f}, ε_acc={agent.accusation_epsilon:.3f}")
...
def evaluate(agent, env, episodes):
    ...
    acc_eps_backup        = agent.accusation_epsilon
    agent.epsilon         = 0.0
    agent.accusation_epsilon = 0.0     # fully exploit both heads during eval
    rewards, wins = [], 0
    ...
    agent.accusation_epsilon = acc_eps_backup
...
for episode in range(start_episode, NUM_EPISODES):
    ...
    for step in range(MAX_STEPS):
        ...
        # --- Accusation transition ---
        # Reward for "wait" on non-terminal steps is stored as 0.0 (clean signal).
        # Terminal accusation steps carry ±1 from the game outcome.
        # The TD mechanism propagates future returns through Q(wait) naturally.
        if env._last_accusation is not None:
            acc_reward = reward if done else 0.0
            agent.store_accusation(
                env._last_accusation["state"],
                env._last_accusation["action"],
                acc_reward,
                next_state,
                done,
            )
            agent.update_accusation()

        # --- Reveal transitions (accumulate; assign terminal reward later) ---
        ...
        # Update reveal head each step (if buffer has enough)
        agent.update_reveal()
    ...
    # Now that we know the terminal reward, assign it back to all reveal
    # transitions that occurred this episode.
    ...
    if (episode + 1) % 50 == 0:
        ...
        msg = (f"Episode {episode+1:>5} | "
               ...
               f"ε_acc: {agent.accusation_epsilon:.3f} | "
               f"Reveal buf: {len(agent.reveal_memory):>4} | "
               f"Acc buf: {len(agent.accusation_memory):>5}")
        logging.info(msg)
        print(msg)
